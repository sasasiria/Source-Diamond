...
@Client.on_message(command(["تشغيل","play","صلاح","شغل", f"play@{BOT_USERNAME}"]) & other_filters)
@AssistantAdd
async def play(c: Client, m: Message):
    ...
    if do.count("left") or do.count("Bad Request: user not found"):
        await m.reply_text(f" ** ┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉**\nعذرا عزيزي انت غير مشترك بقناة البوت @{UPDATES_CHANNEL}\n» **اشتࢪك بقناة البوت لتستطيع تشغيل الاغاني**")
        InlineKeyboardButton(text="اضغط هنا للاشتراك", url=f"https://t.me/{UPDATES_CHANNEL}"),
    ...
    if m.sender_chat:
        return await m.reply_text("__انت مشرف __مجهول !\n\n» العودة إلى حساب المستخدم من حقوق المشرف.")
    try:
        aing = await c.get_me()
    except Exception as e:
